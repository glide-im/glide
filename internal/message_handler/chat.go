...
func (d *MessageHandler) handleChatMessage(c *client.Info, m *messages.GlideMessage) error {
	...
	// 对方不在线, 下发确认包
	if !d.client.IsOnline(client.NewID2(msg.To)) {
...
func (d *MessageHandler) handleChatRecallMessage(c *client.Info, msg *messages.GlideMessage) error {
	return d.handleChatMessage(c, msg)
}

func (d *MessageHandler) ackNotifyMessage(c *client.Info, mid int64) error {
...
func (d *MessageHandler) ackChatMessage(c *client.Info, mid int64) error {
...
func (d *MessageHandler) dispatchOffline(c *client.Info, message *messages.GlideMessage) error {
	logger.D("dispatch offline message %v %v", c.ID, message)
	return nil
}

// dispatchOnline 接收者在线, 直接投递消息
func (d *MessageHandler) dispatchOnline(c *client.Info, msg *messages.ChatMessage) error {
	receiverMsg := msg
	msg.From = c.ID.UID()
